...
void Scene3::init()
{
	...
	cout << "loading tricky1 " << colorTexture.loadFromFile("images/tricky1_2.png", TEXTURE_PIXEL_FORMAT_RGBA) << endl;
	colorTexture.setMinFilter(GL_NEAREST);
	colorTexture.setMagFilter(GL_NEAREST);
	cout << "loading tricky1 mask " << maskTexture.loadFromFile("images/tricky1_2_mask.png", TEXTURE_PIXEL_FORMAT_L) << endl;
	...
	bridges = MaskedTexturedQuad::createTexturedQuad(geomBridge, texCoordsBridge, bridgeProgram);
	...
	//bridgesTextureMask.saveBMP("bridge.bmp");
}

void Scene3::stop() {
	SoundManager::instance().stop(MUSIC3, backgroundMusicID);
}
...
void Scene3::update(int deltaTime)
{
	...
	else if (!backgroundMusicPlaying && !SoundManager::instance().isPlaying(MUSIC3, backgroundMusicID)) {
		backgroundMusicID = SoundManager::instance().play(MUSIC3, true);
...
void Scene3::render()
{
	...
	bridgeProgram.use();
	bridgeProgram.setUniformMatrix4f("projection", projection);
	bridgeProgram.setUniform4f("color", 1.0f, 1.0f, 1.0f, 1.0f);
	modelview = glm::mat4(1.0f);
	modelview = glm::translate(modelview, glm::vec3(.0f, .0f, .0f));
	bridgeProgram.setUniformMatrix4f("modelview", modelview);
	bridges->render(bridgeProgram, bridgeColorTexture, bridgesTextureMask);
	...
	if (win || lose) {
		//bridgesTextureMask.saveBMP("bridge_end.bmp");
		simpleTexProgram.use();
		simpleTexProgram.setUniformMatrix4f("projection", projection);
		simpleTexProgram.setUniform4f("color", 1.0f, 1.0f, 1.0f, 1.0f);
		modelview = glm::mat4(1.0f);
...
void Scene3::initShaders()
{
	...
	bridgeProgram.init();
	bridgeProgram.addShader(vShader);
	bridgeProgram.addShader(fShader);
	bridgeProgram.link();
	if (!bridgeProgram.isLinked())
	{
		cout << "Shader Linking Error" << endl;
		cout << "" << bridgeProgram.log() << endl << endl;
	}
	bridgeProgram.bindFragmentOutput("outColor");
	vShader.free();
	fShader.free();
}
