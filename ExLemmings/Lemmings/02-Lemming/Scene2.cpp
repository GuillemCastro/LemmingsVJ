...
void Scene2::update(int deltaTime)
{
	...
	lose = lose || lemmingAtExit() || ((SCENE2_MAX_TIME - (currentTime / 1000)) <= 0);
...
void Scene2::render()
{
	...
	int secondsLeft = SCENE2_MAX_TIME - (currentTime / 1000);
	if (secondsLeft < 0)
		secondsLeft = 0;
	char buff[100];
...
void Scene2::checkLemmingSelected() {
	Lemming* selectedLemming;
	bool selected = false;
	for (int i = 0; i < 10; ++i) {
